...
export const ChangePassword = () => {
  ...
  return (
    <div className={styles.container}>
      ...
      <span className={styles.containerChange} onClick={() => setVisible(true)}>
        Изменить
      </span>
...
const Popup = ({ setVisible }: { setVisible: () => void }) => {
  ...
  const validate = (pass1: string, pass2: string, newPass: string) => {
    if (pass1 !== pass2) {
      setErrors((prev) => ({
        ...
        second: "Пароли должны совпадать",
      }));
      return false;
    }
    if (newPass.length < 6) {
      setErrors((prev) => ({
        ...prev,
        new: "Пароль должен быть не меньше 6 символов",
      }));
      return false;
    }
    return true;
  ...
  const changePassword = useChangePassword({
    onError: (error) =>
      setErrors({ first: error, second: undefined, new: error }),
